...
const getSentiment = async (userURL) => { //posting the userurl so that the server can get the data. 
  console.log(userURL);
  let response = await fetch("http://localhost:8080/getData", { //since the server is in 8080 and the dev is on 3000, need to include the entire url instead of just the /getData route
  method: "POST",
